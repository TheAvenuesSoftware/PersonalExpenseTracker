...
//         // Async function to set up Redis and create the session store
...
//                     });            
//                 // Optional logging if your variables are defined
//                     if (typeof consoleLog !== 'undefined' && consoleLog === true) {
//                         // Make sure trace() is defined; if not, you can simply log a message
...
//             }
//             // setupRedisStore();
//         // // Export the setup function so you can use it in your main app
//         //     export default setupRedisStore;
// // %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
//     // EPXRESS
//         // EXPRESS-SESSION
//             // LAUNCH SESSION MANAGEMENT
//                 import session from "express-session";
//                 function setupExpressSessionManagement(){
//                     app.use(session({
//                         store: redisStore,
...
// //     const task1 = setupRedis(); // Simulated API call
// //     const task2 = setupExpressSession(); // Email dispatch
// //
// //     try {
// //         // Promise.all()
// //             // // Promise.all() Short-Circuits on Rejection - If any promise rejects, Promise.all() immediately stops waiting for others.
...
// //                 const [result1, result2] = await Promise.allSettled([task1, task2]);
// //             // - Promise.allSettled() ensures all tasks finish, even if one fails. - Each task logs its result—whether fulfilled (status: "fulfilled") or failed (status: "rejected").
// //         console.log("✅ All tasks completed:", result1, result2);
...
// 🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝
    ...
    const logFileName = `logs/request_${myDate.toISOString().replace(/:/g, '-')}.txt`;
    // Write the request data to a log file
        fs.writeFile(logFileName, reqString, (err) => {
            if (err) {
                console.error(`🟨 ${trace("catch-all")}🔴 Error writing request log:`, err);
            } else {
                console.log(`🟨 ${trace("catch-all")}🟢 Request logged: ${logFileName}`);
            }
        });
// 🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝🔝
...
// %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    // MOUNT EXTERNAL ROUTERS
        if(consoleLog===true){console.log(`${trace("mount external routers")}\nMounting routers.`);}
        ...
        import dbRouter, * as dbFunctions from "./src/SQLite_ServerSide.mjs";
            ...
            if(dbFunctions.SQLite_ServerSideMJSisLoaded() === true){
                ...
                console.log(`🔴 ${trace("mount external routers")}\nError.`);
            }
            dbRouter.stack.forEach((route) => {
                if (route.route) {
                    routerMounted = true;
