...
//         const JWT_KEY = process.env.JWT_KEY || sessionKey;
//         const authenticateUser = (req, res, next) => {

//             console.log(`${trace()}\nAuthenticating user.`);

//             // if(typeof req === "undefined"){
//             //     // req.user = { id: null, name: "Guest", role: "anonymous" }; // Assign guest user
//             //     console.log(`${trace()}\n🔴 req.headers:-\n`,"=== undefined");
//             //     return next();
//             // }else{
//             //     console.log(`${trace()}\nreq.headers:-\n`,req.headers);
...
//             // ✅ Define `authHeader` at the top to avoid ReferenceError
//                 const authHeader = req.headers?.authorization || null;
//             // ✅ Define `sessionCookie` at the top to avoid ReferenceError
//                 const sessionCookie = req.headers?.cookie || null;
        
//             if(typeof authHeader === "undefined" || authHeader === null){
//                 console.log(`${trace()}\n[authHeader] === undefined || null`);
//             }else{
//                 console.log(`${trace()}\n[authHeader] === `,authHeader);
//             }
//             if(typeof sessionCookie === "undefined" || sessionCookie === null){
//                 console.log(`${trace()}\n[sessionCookie] === undefined || null`);
//             }else{
...
//             // Silent session regeneration (if valid session exists)
//             if (sessionCookie === "valid_session") {
//                 console.log(trace(),'\n🚦');
//                 req.user = { id: "123", name: "Donald" }; // Mock user
//                 return next();
//             }
...
const authenticateUser = (req, res, next) => {
    ...
    if (!authHeader && !sessionCookie) {
        return next(); // Allow guest users to continue
    }

    let token = authHeader?.startsWith("Bearer ") ? authHeader.split(' ')[1] : null;
    console.log(trace(), 'Token:', token);
    ...
    if (token) {
        try {
            ...
            console.log(trace(), '🔴 Invalid token');
            return res.status(403).json({ error: "Invalid token" });
        }
    }

    if (sessionCookie === "valid_session") {
        req.user = { id: "123", name: "Donald" };
        return next();
    }
